...
//        headerStyle.setFillBackgroundColor(IndexedColors.RED.index);//设置图案背景色
    ...
    public static ResponseEntity<byte[]> exportEmp2(List<Teacher> teachersList) {
        //1.创建一个excel文档
        HSSFWorkbook hssfWorkbook = new HSSFWorkbook();
        ...
        SummaryInformation summaryInformation = hssfWorkbook.getSummaryInformation();
        //文档标题
        summaryInformation.setAuthor("教职工疫情防控信息");
        //文档创建时间
        summaryInformation.setCreateDateTime(new Date());
        //文档备注
        summaryInformation.setComments("教职工疫情防控信息");
...
//        headerStyle.setFillBackgroundColor(IndexedColors.RED.index);//设置图案背景色
        ...
        //这里的m/d/yy 相当于yyyy-MM-dd
        dateCellStyle.setDataFormat(HSSFDataFormat.getBuiltinFormat("m/d/yy"));
        HSSFSheet sheet = hssfWorkbook.createSheet("教职工疫情防控信息表");
        //设置每一列的宽度
        sheet.setColumnWidth(0,5*256);
        sheet.setColumnWidth(1,12*256);
        sheet.setColumnWidth(2,10*256);
        ...
        HSSFCell c9 = r0.createCell(9);
        c9.setCellStyle(headerStyle);
        c9.setCellValue("是否留校中国学生");
        HSSFCell c10 = r0.createCell(10);
        c10.setCellStyle(headerStyle);
        c10.setCellValue("是否武汉籍学生");
        HSSFCell c11 = r0.createCell(11);
        c11.setCellStyle(headerStyle);
        c11.setCellValue("是否湖北籍学生");
        HSSFCell c12 = r0.createCell(12);
        c12.setCellStyle(headerStyle);
        c12.setCellValue("14天内与武汉疫区人员接触");
        HSSFCell c13 = r0.createCell(13);
        c13.setCellStyle(headerStyle);
        c13.setCellValue("14天内与湖北疫区（不含武汉）人员接触");
        HSSFCell c14 = r0.createCell(14);
        c14.setCellStyle(headerStyle);
        c14.setCellValue("是否在武汉");
        HSSFCell c15 = r0.createCell(15);
        c15.setCellStyle(headerStyle);
        c15.setCellValue("是否在湖北（不含武汉)");
        HSSFCell c16 = r0.createCell(16);
        c16.setCellStyle(headerStyle);
        c16.setCellValue("是否今天返校");
        HSSFCell c17 = r0.createCell(17);
        c17.setCellStyle(headerStyle);
        c17.setCellValue("是否疑似");
        HSSFCell c18 = r0.createCell(18);
        c18.setCellStyle(headerStyle);
        c18.setCellValue("是否确诊感染");
        ...
        for (int i = 0; i < teachersList.size(); i++) {
            ...
            row.createCell(1).setCellValue(teacher.getCode());
            row.createCell(2).setCellValue(teacher.getCollege());
            row.createCell(3).setCellValue(teacher.getDate());
            row.createCell(4).setCellValue(teacher.getName());
            row.createCell(5).setCellValue(teacher.getPhone());
            row.createCell(6).setCellValue(teacher.getProvince());
            row.createCell(7).setCellValue(teacher.getCity());
            row.createCell(8).setCellValue(teacher.getArea());
            row.createCell(9).setCellValue(teacher.getStayAtSchool());
            row.createCell(10).setCellValue(teacher.getIfWuhanperson());
            row.createCell(11).setCellValue(teacher.getIfHubeiperson());
            row.createCell(12).setCellValue(teacher.getWithWuhanMeet());
            row.createCell(13).setCellValue(teacher.getWithHubeiMeet());
            row.createCell(14).setCellValue(teacher.getInWuhan());
            row.createCell(15).setCellValue(teacher.getInHubei());
            row.createCell(16).setCellValue(teacher.getBackToSchool());
            row.createCell(17).setCellValue(teacher.getSuspect());
            row.createCell(18).setCellValue(teacher.getInfect());
        }
        ...
        try {
            //将数据表这几个中文的字转码 防止导出后乱码
            headers.setContentDispositionFormData("attachment",
                    new String("教职工疫情防控信息表.xls".getBytes("UTF-8"),"ISO-8859-1"));
            headers.setContentType(MediaType.APPLICATION_OCTET_STREAM);
            hssfWorkbook.write(stream);
        } catch (IOException e) {
            e.printStackTrace();
